...
\begin{frame}{Perceived risks (PR)}
	\begin{itemize}
		...
		\begin{equation*}
			...
			\end{split}
		\end{equation*}
		\item \textcolor{red}{To the agent: \textbf{conditional volatility} under FIRE}
		\begin{equation*}
			\begin{split}
				Var^*_{i,c,t}(\Delta y_{i,c,t+1}) & =\sigma^2_{c,t+1|t}
			\end{split}
		\end{equation*}
		\item \textcolor{blue}{To econometricians: \textbf{unconditional volatility} } 
		\begin{equation*}
			\begin{split}
...
\begin{frame}{Appendix: calibration of the objective model}
	...
	\begin{table}[p]
		\centering
			...
			\begin{tabular}{ll}
				\hline 
				(`risk', $\sigma_\psi$)                        & 0.15  \\
				(`risk', $\sigma_\theta$)                        & 0.1   \\
				(`risk', $U2U$)                         & 0.18  \\
			(`risk', $E2E$)                         & 0.96  \\
			(`initial condition', $\sigma_\psi^{\text{init}}$)     & 0.629 \\
		(`initial condition', $b^{\text{init}}$)        & 0.554 \\
	(`initial condition', `bequest ratio') & 0     \\
	(`life-cycle', $T$)                     & 40    \\
	(`life-cycle', $L$)                     & 60    \\
	(`life-cycle', $1-D$)                   & 0.994 \\
	(`preference', $\rho$)                     & 2     \\
	(`preference', $\beta$)                     & 0.98  \\
	(`policy', $\lambda$)                         & 0     \\
	(`policy', $\lambda_{SS}$)                     & 0     \\
	(`policy', `transfer')                  & 0     \\
	(`policy', $\mu$)                         & 0.15 \\
