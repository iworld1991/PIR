...
# ## A life-cycle consumption  model with a markov state 
#
# - author: Tao Wang
# - date: Feb 2022
...
#   
# - The consumption is a function over wealth in hand to permanent income ratio, m and markov state 
...
class LifeCycle:
    ...
    def terminal_solution(self):
        ...
        k = len(self.a_grid)
        n_z =len(self.P)
        σ_init = np.empty((k,n_z))
        m_init = np.empty((k,n_z))
        
        for z in range(n_z):
                m_init[:,z] = np.exp(np.linspace(np.log(1e-6),
...
def EGM(mϵ_in,
    ...
    σ = lambda m,z: interp(mϵ_in[:,z],σ_in[:,z], m)
        ...
        for z in range(n_z):
            ...
            for z_hat in range(n_z):
                ...
                for psi_shk in psi_shk_draws:
                    for eps_shk in eps_shk_draws:
                        ...
                        if age <=lc.T-1:   #till say 39, because consumption policy for t=40 changes
                            ...
                            Γ_hat = 1.0
                            c_hat = σ(R/(G*Γ_hat) * a + (Y_R+transfer),z_hat)
                            utility = (G*Γ_hat)**(1-ρ)*u_prime(c_hat)
                ...
                if adjust_prob ==1.0:
                    ...
                    σ_out[i, z] =  adjust_prob/(1-LivProb*β*R*(1-adjust_prob))*u_prime_inv(β * R* Ez)

    # Calculate endogenous asset grid
    mϵ_out = np.empty_like(σ_out)

    for z in range(n_z):
        mϵ_out[:,z] = a_grid + σ_out[:,z]
    ...
    for i, a in enumerate(a_grid):
        for z in range(n_z):
            ...
            if adjust_prob ==1.0:
                ...
                σ_out[i, z] =  adjust_prob/(1-β*R*(1-adjust_prob))*u_prime_inv(β * R* Ez)
...
def solve_model_backward_iter(model,        # Class with model information
    ...
    n_a = σ_vec.shape[0]
    n_z = len(model.P)
    mϵs_new =  np.empty((model.L,n_a,n_z),dtype = np.float64)
    σs_new =  np.empty((model.L,n_a,n_z),dtype = np.float64)
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        for k,sigma_psi in enumerate(sigma_psi_ls):
            m_plt,c_plt = ms_stars[k][i,:,1],σs_stars[k][i,:,1]
            axes[x].plot(m_plt,
                         c_plt,
...
if __name__ == "__main__":
    ...
    c_stars = np.flip(σs_star[:,:,0],axis=0).T
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        z_l = 0
        z_h = 1
        m_plt_l,c_plt_l = ms_stars_ar[0][i,:,z_l],σs_stars_ar[0][i,:,z_l]
        m_plt_h,c_plt_h  = ms_stars_ar[0][i,:,z_h],σs_stars_ar[0][i,:,z_h]
        axes[x].plot(m_plt_l,
                     c_plt_l,
                     '--',
                     label ='bad',
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        z_l = 0
        z_h = 1
        m_plt_l,c_plt_l = ms_stars_sv[0][i,:,z_l],σs_stars_sv[0][i,:,z_l]
        m_plt_h,c_plt_h = ms_stars_sv[0][i,:,z_h],σs_stars_sv[0][i,:,z_h]
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        m_plt,c_plt = ms_star_basic[i,:,z_l],σs_star_basic[i,:,z_l]
        axes[x].plot(m_plt,
                     c_plt,
                     label = 'objective',
                     lw=3)
        z_l = 0
        z_h = 1
        m_plt_l,c_plt_l = ms_stars_sv[0][i,:,z_l],\
                          σs_stars_sv[0][i,:,z_l]
        m_plt_h,c_plt_h = ms_stars_sv[0][i,:,z_h],\
                          σs_stars_sv[0][i,:,z_h]
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        z_l = 0
        z_h = 1
        m_plt_u, c_plt_u = ms_stars_uemkv[0][i,:,z_l],σs_stars_uemkv[0][i,:,z_l]
        m_plt_e, c_plt_e = ms_stars_uemkv[0][i,:,z_h],σs_stars_uemkv[0][i,:,z_h]
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        age = lc_basic.L-year
        i = lc_basic.L-age
        m_plt_l,c_plt_l = ms_stars_cr[0][i,:,z_l],\
                          σs_stars_cr[0][i,:,z_l]
        m_plt_h,c_plt_h = ms_stars_cr[0][i,:,z_h],\
                          σs_stars_cr[0][i,:,z_h]
...
if __name__ == "__main__":
    ...
    for x,year in enumerate(years_left):
        ...
        ## countercyclical
        m_plt_l,c_plt_l = ms_stars_cr[0][i,:,z_l],σs_stars_cr[0][i,:,z_l]
        m_plt_h,c_plt_h = ms_stars_cr[0][i,:,z_h],σs_stars_cr[0][i,:,z_h]
        axes[x].plot(m_plt_l, ## 0 indicates the low risk state
                 c_plt_l,
                 '--',
                 label ='ue + high risk',
