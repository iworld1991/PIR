...
\import{./}{appendix_other_hist}
\import{./}{appendix_income.tex}

\import{./}{appendix_cyclicality.tex}

\import{./}{appendix_experience.tex}

\import{./}{appendix_covariates.tex}
...
This effect is clearly seen from the time series plot of monthly wage volatility in Figure \ref{fig:volatility_monthly_seam}, where there is always a spike in the size of volatility between December to January in the sample period.\footnote{Note that the only exception is January 2017, for which no monthly growth rate is not available due to reshuffling of the SIPP sample.}

 \begin{figure}[!ht]
    	\caption{Estimated monthly wage volatility}
    	\label{fig:volatility_monthly_seam}
    	\begin{center}
    	\adjustimage{max size={0.8\linewidth}}{figures/log_wage_shk_gr_sd.png}
    	\end{center}
    	\floatfoot{Note: this figure plots the monthly wage volatility as defined in Equation \ref{Eq:definition_volatility} for the entire selected sample, estimated from SIPP.} 
    \end{figure}
...
\end{equation}
 ...
 \begin{figure}[!ht]
    	...
    	\end{center}
    	\floatfoot{Note: this figure plots the 3-month moving average of the estimated monthly permanent and transitory risks (variance) using the SIPP panel data on wage between 2013m1-2019m12. }
    \end{figure}
...
\begin{figure}[!ht]
    	...
    	\begin{center}
    	\adjustimage{max size={0.5\linewidth}}{figures/high_freq_cdf_all.png} \\ \adjustimage{max size={0.5\linewidth}}{figures/high_freq_cdf_by_group.png}
    	\end{center}
...
\subsubsection{Estimated risks at a lower frequency}
...
For the years with identified risks, the estimated risks at annual frequency seems to be much larger than that commonly seen in the literature, as summarized in Table \ref{tab:risks_literature}. In particular, the size of the permanent shock ranges from 27\% to 41\%, in contrast to the standard estimation of 10-15\%. And the transitory risks are estimated to be around 25\%, which also exceeds the standard estimates of 10\% to 20\%. 
 ...
 \begin{figure}[!ht]
    	...
    	\begin{center}
    	\adjustimage{max size={0.5\linewidth}}{figures/permanent-transitory-risk-yearly.jpg}
    	\end{center}
    	\floatfoot{Note: this figure plots the estimated yearly permanent and transitory risks (variance) using the SIPP panel data on wage between 2013m1-2019m12.}
    \end{figure}
...
The similar issue can be seen from quarterly estimates using quarterly growth of average wage rates.(See Figure \ref{fig:decomposed_quarterly}) Reminiscent of the seasonal spike in the monthly volatility in January, there is a similar spike in the first quarter every year in the sample.    
 ...
 \begin{figure}[!ht]
    	...
    	\begin{center}
    	\adjustimage{max size={0.5\linewidth}}{figures/permanent-transitory-risk-quarterly.jpg}
    	\end{center}
    	\floatfoot{Note: this figure plots the estimated quarterly permanent and transitory risks (variance) using the SIPP panel data on wage between 2013m1-2019m12.}
    \end{figure}
